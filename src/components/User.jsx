...
export default function User({ username, userAvatar, company, location }) {
  return (
    <div className="bg-secColor/30 border border-secColor h-36 md:h-72 px-5 md:px-10 rounded-3xl flex items-center">
      <div className="flex items-start justify-between w-full">
        <div className="flex items-start gap-8 md:gap-14">
          ...
          <div>
            <div className="md:text-2xl font-semibold pb-5">@{username}</div>
            {company && (
            ...
            {location && <div className="text-brandBlue w-[80%] md:w-full">{location}</div>}
          </div>
        </div>
